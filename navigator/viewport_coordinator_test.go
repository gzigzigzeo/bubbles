...
func TestCoordinator_FocusFollowScroll(t *testing.T) {
	...
	nav.ViewportCoordinator().SetHeight(3)
	_ = nav.FocusFirst()

	require.Equal(t, 0, nav.ViewportCoordinator().YOffset())
	...
	require.Equal(t, 3, nav.focused)
	require.Equal(t, 1, nav.ViewportCoordinator().YOffset())

	got := nav.CursorLine()
	require.GreaterOrEqual(t, got, nav.ViewportCoordinator().YOffset())
	require.Less(t, got, nav.ViewportCoordinator().YOffset()+nav.ViewportCoordinator().Height())
}
...
func TestCoordinator_BoundaryScrollUp(t *testing.T) {
	...
	nav.ViewportCoordinator().SetHeight(2)
	_ = nav.FocusFirst()
...
func TestCoordinator_BoundaryScrollDown(t *testing.T) {
	...
	sendKey(t, nav, "down")
	require.Equal(t, 2, nav.ViewportCoordinator().YOffset())
...
func TestCoordinator_StopsAtScreenEdge(t *testing.T) {
	...
	nav.ViewportCoordinator().SetHeight(3)
	_ = nav.FocusFirst()
	nav.ViewportCoordinator().SetYOffset(2)
