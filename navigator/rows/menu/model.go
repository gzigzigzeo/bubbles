...
func New[T comparable](name string, value T, description string, msg tea.Msg) *Model[T] {
	if msg == nil {
		panic("menu.New: msg must not be nil")
	}

	rowModel := &Model[T]{
		StatefulStyles: row.StatefulStyles[RowStyles]{},
	...
	rowModel.SetStyles(DefaultStyles())
...
func (m *Model[T]) View() tea.View {
	...
	if m.marked {
		nameStyle = styles.MarkedName
	}

	name := nameStyle.Render(m.name)
...
// markRender returns the mark glyph when the row is marked, or the configured
// missing-mark placeholder otherwise.
func (m *Model[T]) markRender(styles RowStyles) string {
	if m.marked {
		return styles.Mark.Render()
	}

	return styles.NoMark.Render()
}
